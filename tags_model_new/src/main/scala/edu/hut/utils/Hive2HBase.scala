package edu.hut.utils

import org.apache.hadoop.conf.Configuration
import org.apache.hadoop.fs.Path
import org.apache.hadoop.hbase.client.{ClusterConnection, ConnectionFactory, HRegionLocator}
...
object Hive2HBase {
  ...
  def hfile2HBase(job: Job, table: String, hfilePath: String): Unit = {
    ...
    if (!admin.tableExists(tableName)) {
      val descriptor = new HTableDescriptor(TableName.valueOf(tableName.toString))
      descriptor.addFamily(new HColumnDescriptor(defaultHBaseCF))
